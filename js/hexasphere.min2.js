function PRingBufferGeometry(e, t, a, i, n, r) {
	...
	R = 1 / a;
	for (s = 0; s <= i; s++) {
		for (l = 0; l <= a; l++) o = n + l / a * r,
		g.x = E * Math.cos(o),
		g.y = E * Math.sin(o),
		h.setXYZ(m, g.x, g.y, g.z),
		f.setXYZ(m, 0, 0, 1),
		T.x = l * R,
		T.y = s * x,
		p.setXY(m, T.x, T.y),
...
function Planet(e, t, a, i) {
	...
	function r(e) {
		mouse_is_down = !1,
		h.state == h.IDLE && (M > 6 || isMobile.any && d())
	}
	function o(e) {
		if (e.preventDefault(), e.touches) {
			var t = e.touches[0];
	...
	function s(e, t) {
		...
		H.x = e,
		H.y = t,
		isMobile.any ? (A.x = 2 * (H.x / f.originalSize.width - .5), A.y = 2 * -(H.y / f.originalSize.height - .5)) : (A.x = 2 * (H.x / f.width - .5), A.y = 2 * -(H.y / f.height - .5))
	}
	function l() {
		if (h.state == h.IDLE) {
			var e, t;
			return P.setFromCamera(A, m),
			e = P.ray.intersectSphere(b),
			e && (t = h.getClosestLocation(e)),
			!(!t || t == h.location_current) && (u(t), !0)
		}
	}
	function d() {
		var e, t;
	...
	function u(t, a) {
		if (t.years[PlanetData.YEAR_ID] && (!window.earthDemo || !window.earthDemo.running)) {
			...
			t.briefs_text = PlanetData.hasLocationAnyBriefTexts(O, !0),
	...
	function c(t) {
		...
		TweenLite.to(U, 1.5, {
			...
			ease: Sine.easeInOut,
			onUpdate: function() {
				THREE.Quaternion.slerp(r, n, U.quaternion, U._anim_progress_),
				U.updateMatrix()
			},
			onComplete: function() {
	...
	this.getLocationBriefs = function(e, t) {
		...
		h.locationsFullData ? (n = a.brief_list = h.locationsFullData[e] || {},
		!1 != !!n[PlanetData.YEAR_ID] && t(n[PlanetData.YEAR_ID].briefs)) : i().done(function(i) {
			h.locationsFullData = i,
			n = a.brief_list = h.locationsFullData[e] || {},
			!1 != !!n[PlanetData.YEAR_ID] && t(n[PlanetData.YEAR_ID].briefs)
		})
	},
	this.drawPoints(1.1 * v),
	...
	this.update = function() {
		if (R = m.position.distanceToSquared(THREE.Vector3.ZERO) - 100, g.lookAt(m.getWorldPosition()), h.static_container.updateMatrix(), this.grid_shpere.rotation.y += 5e-4 * h.speedMultiple, this.orbits) for (var t = 0; t < this.orbits.length; t++) {
			var a = this.orbits[t];
			a.rotation.x += a._increment.x * h.speedMultiple,
			a.rotation.y += a._increment.y * h.speedMultiple,
			a.rotation.z += a._increment.z * h.speedMultiple
		}
		if (this.sputniks) for (var t = 0; t < this.sputniks.length; t++) if (! (this.frozen && this.frozen.indexOf("sputnik" + t) > -1)) {
		...
		if (this._PlanetCommentPopup && this._PlanetCommentPopup.update(), this.state != this.IDLE || isMobile.any || l(), this.demoTargetLocation) {
			...
			this.demoTargetLocation.position.clone().applyEuler(new THREE.Euler(0, e.camera.getWorldRotation().y, 0, "XYZ")).projectOnPlane(o).x > Math.PI && (this.demoMoveToHexagon(this.demoTargetLocation, h.demoCallback), h.demoTargetLocation = null)
		}
	},
	h.tile_current = null;
	...
	f.addEventListener("touchstart", n, !1),
	f.addEventListener("touchmove", o, !1),
	f.addEventListener("touchend", r, !1),
	f.addEventListener("mousedown", n, !1),
	f.addEventListener("mousemove", o, !1),
	f.addEventListener("mouseup", r, !1);
	var y, S, L, D, O = null,
	...
	U.up = new THREE.Vector3(0, 1, 0),
	e.scene.add(U),
	U.matrixAutoUpdate = !1,
	this.changeSpeed = function(t) {
		e.controls.autoRotateSpeed = -.002 * t,
		h.speedMultiple = t
	};
	var W = !0;
	this.demoStartLocation = function(e, t, a) { ! 1 === e && (e = 0);
		var i = Object.assign({},
		h.planetLocations.current_locations[e]);
		a && W && (W = !1, i.position.x -= 2, i.position.y -= 1),
		this.changeSpeed(4),
		h.demoTargetLocation = i,
		h.demoCallback = t
	},
	this.demoMoveToHexagon = function(t, a) {
		...
		t.position.clone();
		U.lookAt(e.camera.getWorldPosition()),
		U.setRotationFromEuler(e.camera.getWorldRotation());
		var n = U.quaternion.clone();
		e.camera.getWorldPosition().clone().applyEuler(e.camera.getWorldRotation());
		U.updateMatrix();
		var r = e.camera.position.length();
		U.add(e.camera),
		e.camera.position.set(0, 0, r),
		e.camera.rotation.set(0, 0, 0),
		p.updateMatrixWorld(),
		U._anim_progress_ = 0,
		TweenLite.to(U, 1.5, {
			...
			ease: Sine.easeInOut,
			onUpdate: function() {
				THREE.Quaternion.slerp(n, i, U.quaternion, U._anim_progress_),
				U.updateMatrix()
			},
			onComplete: function() {
				h.demoStartZ = e.camera.position.z,
				h.demoLocation = t,
				TweenLite.to(e.camera.position, 1.2, {
					z: 1.15 * v,
					ease: Sine.easeIn,
	...
	this.demoMoveToSputnik = function(t, a) {
		...
		e.controls.autoRotate = !1,
		h.commentToggle(!1),
		U.lookAt(window.PLANET.sputniks[t].children[0].getWorldPosition());
		var i = U.quaternion.clone();
		U.lookAt(e.camera.getWorldPosition()),
		U.setRotationFromEuler(e.camera.getWorldRotation());
		var n = U.quaternion.clone();
	...
	this.googlePosToUV = function(e, t) {
		...
		var i = 1 - (parseFloat(e) + 90) / 180; (i += .014) > 1 && (i -= 1);
...
function PlanetLocations(e) {
	...
	var p = Utils.createHexagon(0.08, !1, void 0),
	m = Utils.createHexagon(0.1, !1, void 0, .02);
	p.add(m);
	...
	this.init = function(t) {
		...
		for (var d, h = new THREE.Geometry,
	...
	this.show = function(a, i) {
		var r = PlanetData.years[a];
		r.locations_container || (r.locations_container = this.init(a)),
		o = r.locations_container,
	...
	g = w;
	g.map = e.textureLoader.load(PlanetData.textures_path + "lightray.jpg"),
...
function PlanetPointed(e) {
	...
	for (var a = [], i = 0; i < 2; i++) {
		...
		n.color = new THREE.Color(3257463),
		n.map = t,
		n.depthWrite = !1,
		n.transparent = !0,
		n.opacity = 0,
		...
		n.t_ = r * Math.PI * 2,
		n.speed_ = .04,
		n.min_ = .2 * Math.random() + .5,
		n.delta_ = .1 * Math.random() + .1,
		n.opacity_coef_ = 1,
		a.push(n)
	}
	var o, s;
	this.init = function(t) {
		...
		l = PlanetData.years[t].earth_image, d = 0; d < 250; d++) for (var u = 250 * (1 - Math.sin(d / 250 * Math.PI)) / 250 + .5, c = 0; c < 250; c += u) {
			var h = c / 250,
			f = d / 250,
			p = l.isLandByUV(h, f);
			if (p) {
				...
				m.positions.push(o.x),
				m.positions.push(o.y),
				m.positions.push(o.z)
			}
		}
		for (var r = 0; r < n.length; r++) {
			...
			v.addAttribute("position", new THREE.BufferAttribute(E, 3)),
		...
		for (var T, x, R, H, A = Utils.createHexagon(.12, !1, s, .025), M = new THREE.Geometry, b = .01, P = (Math.PI, !1), y = .5 - Math.PI; y < -.6; y += .04) {
			R = .5 * Math.cos(y) + .5,
			P = !P,
			b = Math.abs(.01 / Math.sin(R * Math.PI)),
			T = Math.floor(1 / b);
			for (var S = 0; S < T; S++) x = .5 + (T / 2 - S - (P ? .5 : 0)) * b,
			H = R,
			Math.random() > .25 || l.isLandByUV(x, H) || l.isLandByUV(x - .02, H) || l.isLandByUV(x + .02, H) || l.isLandByUV(x, H - .02) || l.isLandByUV(x, H + .02) || (Utils.setFromSpherical(e.radius * (.97 - .01 * Math.random()), x, H, A.position), A.lookAt(THREE.Vector3.ZERO), A.updateMatrix(), A.updateMatrixWorld(), M.merge(A.geometry, A.matrixWorld))
		}
	...
	this.show = function(t, i) {
		var n = PlanetData.years[t];
		n.pointed_sphere || (n.pointed_sphere = this.init(t)),
		o = n.pointed_sphere,
		e.container.add(o);
...
function PlanetCommentPopup(e) {
	...
	this.show = function(e, s, d, u) {
		if ((!window.earthDemo || !window.earthDemo.running) && e) {
			...
			a = !0,
			l.position.copy(s),
			d && (clearTimeout(t), t = setTimeout(function() {
				hide(u)
			},
			d))
		}
	...
	return this.showRandom = function() {
		...
		i.getLocationBriefs(e.location_id,
		function(t) {
			if (t && t.length) {
...
function ProjectiveImage(e, t) {
	...
	this.isLand = function(e, t) {
		...
		return null == n && (n = a.getImageData(0, 0, i.width, i.height)),
		0 === n.data[4 * (o * n.width + r)]
	},
...
THREE.RenderPass.prototype = Object.assign(Object.create(THREE.Pass.prototype), {
	...
	render: function(e, t, a, i, n) {
		...
		var o, s;
		this.clearColor && (o = e.getClearColor().getHex(), s = e.getClearAlpha(), e.setClearColor(this.clearColor, this.clearAlpha)),
...
THREE.POrbitControls = function(e, t) {
	function a(e, t, a) {
		v.x = e,
		v.y = t,
		v.delta_x = a ? 0 : v.x - v.prev_x,
		v.delta_y = a ? 0 : v.y - v.prev_y,
		v.prev_x = v.x,
		v.prev_y = v.y,
		h && (m.phi -= .002 * v.delta_y, m.theta -= .002 * v.delta_x, m.makeSafe())
	}
	function i(e) { ! 1 !== c.enabled && (e.preventDefault(), h = !0, a(e.clientX, e.clientY, !0))
	}
	function n(e) { ! 1 !== c.enabled && a(e.clientX, e.clientY)
	}
	function r(e) { ! 1 !== c.enabled && (e.preventDefault(), h = !1)
	}
	function o(e) { ! 1 !== c.enabled && !1 !== c.enableZoom && (e.preventDefault(), e.stopPropagation(), e.deltaY < 0 ? s( - c.zoomSpeed) : e.deltaY > 0 && s(c.zoomSpeed))
	}
	function s(e) {
		m.radius += e,
	...
	function d(e) {
		if (!1 !== c.enabled) {
			...
			case 2:
				...
				w.set(0, r),
				g.subVectors(w, E),
				g.y > 0 ? s( - c.zoomSpeed) : g.y < 0 && s(c.zoomSpeed),
				E.copy(w)
			}
		}
	}
	function u(e) { ! 1 !== c.enabled && (h = !1)
	}
	var c = this,
	h = !1,
	f = new THREE.Vector3,
	...
	c.attach = function() {
		...
		window.addEventListener("mouseout", r, !1),
		t.addEventListener("wheel", o, !1),
		m.setFromVector3(e.position),
	...
	c.detach = function() {
		...
		window.removeEventListener("mouseout", r),
		t.removeEventListener("wheel", o)
	},
	c.update = function() {
		p.radius += (m.radius - p.radius) * c.zoomFactor,
		m.theta = Math.max(c.minAzimuthAngle, Math.min(c.maxAzimuthAngle, m.theta)),
		m.phi = Math.max(c.minPolarAngle, Math.min(c.maxPolarAngle, m.phi)),
		!h && c.autoRotate && (m.theta += c.autoRotateSpeed, m.makeSafe()),
		c.enableDamping ? (p.phi += (m.phi - p.phi) * c.dampingFactor, p.theta += (m.theta - p.theta) * c.dampingFactor, p.makeSafe()) : (p.radius = m.radius, p.phi = m.phi, p.theta = m.theta),
...
function(e) {
	function t() {
		...
		l.setPixelRatio(window.earthDemo && window.earthDemo.running ? 1 : o),
		l.setSize(e, t),
		c && (c.setSize(e, t), f.uniforms.resolution.value.set(1 / e, 1 / t), p.uniforms.v.value = 1 / t)
	}
	function a() {
		T || (E.state != E.ANIMATED && v.update(), E.update(), c ? c.render() : l.render(s, u), m && m.update(l), g = window.requestAnimationFrame(a))
	}
	function i() {
		isMobile.any || ($(l.domElement).remove(), l = n.renderer = new THREE.WebGLRenderer({
			preserveDrawingBuffer: !0,
			powerPreference: "high-performance"
		}), l.autoClearColor = new THREE.Color(0, 0, 0, 0), l.setPixelRatio(1), r.append(l.domElement), l.domElement.addEventListener("webglcontextlost",
		function() {
	...
	s.fog = new THREE.Fog(0, 7, 40);
	var l = n.renderer = new THREE.WebGLRenderer({
		preserveDrawingBuffer: !0,
		powerPreference: "high-performance"
	});
	l.autoClearColor = new THREE.Color(0, 0, 0, 0),
	l.setPixelRatio(o);
	var d = l.domElement;
	r.append(d);
	...
	if (!isMobile.any) {
		var h;
		c = new THREE.EffectComposer(l),
		h = new THREE.RenderPass(s, u),
		c.addPass(h),
		h = new THREE.ShaderPass(THREE.RGBShiftShader),
		h.uniforms.amount.value = .0025,
		c.addPass(h);
		var f = h = new THREE.ShaderPass(THREE.FXAAShader);
		c.addPass(h);
		var p = h = new THREE.ShaderPass(THREE.VerticalTiltShiftShader);
		h.uniforms.r.value = .35,
		h.renderToScreen = !0,
		c.addPass(h)
	}
	var m;
	...
	t(),
	window.resizeEarth = t;
	var v = n.controls = new THREE.POrbitControls(u, l.domElement);
	v.minPolarAngle = .15 * Math.PI,
	v.maxPolarAngle = .85 * Math.PI,
	v.enableDamping = !0,
	v.dampingFactor = .063,
	v.autoRotate = !0,
	v.autoRotateSpeed = -.006,
	v.enableZoom = !0,
	v.minDistance = 18,
	v.maxDistance = 20,
	v.zoomSpeed = .2,
	...
	var g, T = !1,
	x = !1;
	l.domElement.addEventListener("webglcontextlost",
	function() {
		isMobile.any || (x = !0, T || i())
	}),
	r.on("animate:start",
	function() {
		...
		function() {
			isMobile.any || (x = !0, T || i())
		}), window.resizeEarth(), c ? c.render() : l.render(s, u)) : x ? (x = !1, i()) : l.setPixelRatio(1),
		a()
	}),
	r.on("animate:stop",
	function() {
		...
		l.clear(!0, !0, !0),
		l.dispose(),
		isMobile.any && (window.navigator.userAgent.indexOf("MSIE ") > 0 || window.navigator.userAgent.match(/Trident.*rv\:11\./) || l.context.isContextLost() || l.forceContextLoss())
	}),
...
Planet.prototype.getAnyClosestLocation = function(e, t) {
	for (var a, i = 1e4,
	n = this.planetLocations.current_locations,
	r = 0; r < n.length; r++) {
		var o = n[r];
		if (!t || !
		function(e) {
...
Planet.prototype.drawPoints = function(e) {
	...
	for (var n = 3 * t.vertices.length,
	r = new Float32Array(n), o = new Float32Array(n), s = new Float32Array(n), l = new THREE.Color(16777215), d = 0; d < t.vertices.length; d++) {
		var u = t.vertices[d],
		c = 3 * d;
		r[c] = u.x,
		r[c + 1] = u.y,
		...
		o[c + 1] = l.g,
		o[c + 2] = l.b,
		s[Math.floor(c / 3)] = .3 * Math.random() + .5
	}
	var h = new THREE.BufferGeometry;
	h.addAttribute("position", new THREE.BufferAttribute(r, 3)),
	h.addAttribute("color", new THREE.BufferAttribute(o, 3)),
	h.addAttribute("alpha", new THREE.BufferAttribute(s, 1)),
...
Planet.prototype.drawParticles = function(e) {
	for (var t = new Float32Array(1800), a = new THREE.Color(263385797), i = new THREE.Spherical, n = new THREE.Vector3, r = 0; r < 600; r++) {
		var o = 3 * r;
		i.radius = e * (1 + .6 * Math.random()),
		i.theta = 8 * Math.random(),
		i.phi = .3 + 2.2 * Math.random(),
		n.setFromSpherical(i),
		t[o] = n.x,
		t[o + 1] = n.y,
		t[o + 2] = n.z
	}
...
Planet.prototype.drawRing = function(e, t, a, i, n) {
	...
	o = new THREE.Matrix4;
	n ? o.setPosition(new THREE.Vector3(0, 0, .29 * t)) : o.makeRotationX(Math.PI / 2),
	r.applyMatrix(o);
...
Planet.prototype.drawOrbitas = function() {
	...
	for (var e = 0; e < this.radius; e += 3) {
		...
		n._increment = new THREE.Vector3(r, o, s),
		this.orbits.push(n)
	}
},
Planet.prototype.drawBG = function() {
	function e(e, a, s, l, c, h) {
		...
		m.position.x = -r / 2 + a / 2 + l / i * r,
		m.position.y = o / 2 - s / 2 - c / n * o,
		m.position.z = .01,
...
Planet.prototype.drawSputniks = function() {
	this.sputniks = [];
	this.addSputnik("space_a.js", 1.2 * this.radius, this.container, new THREE.Vector3( - .5, 0, 0)),
	this.addSputnik("space_a.js", 1.1 * this.radius, this.container, new THREE.Vector3( - .7, 2, 0)),
	this.addSputnik("space_c.js", 1.25 * this.radius, this.container, new THREE.Vector3(.6, 3, -1.2))
},
Planet.prototype.addSputnik = function(e, t, a, i) {
	...
	r.load(PlanetData.textures_path + e,
	function(e, a) {
		var i = new THREE.MeshBasicMaterial({
...
var PlanetData = function() {
	...
	this.year_ids = [];
	for(var pi=0;pi<window.myProTypes.length;pi++){
		//this.years[window.myProTypes[pi]]={earth_url: this.textures_path + "earth_1.png",contour_url: [this.textures_path + "contour1_1.png", this.textures_path + "contour2_1.png"],min_locations: pi};
		this.years[window.myProTypes[pi]]={earth_url: this.textures_path + "earth_1.png",contour_url: [this.textures_path + "contour1_1.png", this.textures_path + "contour2_1.png"],min_locations: pi};
	}
...
THREE.Text = function(e) {
	function t() {
		...
		n.font = a.fontStyle + " " + a.font_height + "px " + a.font,
		n.fillStyle = a.fillStyle,
	...
	d.setSize = function(e, t) {
		...
		i.width = o,
		i.height = s,
		r.repeat.x = d.pow_coef_x = a / o,
		r.repeat.y = d.pow_coef_y = n / s,
		r.offset.y = 1 - r.repeat.y,
		d.scale.set(e, t, 1),
		d.updateMatrix()
	},
	d.setWidth = function(i) {
		var r;
		return "string" == typeof i ? (t(), r = (n.measureText(i).width + a.offset_x + a.border_x) / 100) : r = i,
		d.setSize(r, e.height),
		r
	},
	d.setText = function(e, o) {
		...
		t(),
		n.fillText(e, a.offset_x, d.canvas_vis_height / 2),
		r.needsUpdate = !0
	},
	d.show = function(e) {
		d.scale.x = .001,
...
function() {
	...
	this.createHexagon = function(e, t, a, i) {
		...
		if (t) {
			...
			r.identity().makeRotationX(Math.PI / 2),
			n.applyMatrix(r),
			r.identity().makeRotationY(Utils.PI2 / 6 / 2),
			n.applyMatrix(r)
		}
	...
	this.setFromSpherical = function(e, t, a, i) {
		...
		return n.radius = e,
		n.theta = t * Math.PI * 2 - Math.PI / 2,
		n.phi = a * Math.PI,
		i || (i = new THREE.Vector3),
		i.setFromSpherical(n),
		i
	}
};
Utils.getEventCursorPosition = function(e, t) {
	...
	t ? (t.x = a.clientX, void(t.y = a.clientY)) : {
		x: a.clientX,
		y: a.clientY
	}
},
